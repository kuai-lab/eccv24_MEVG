...
class CrossAttention(nn.Module):
    ...
    def forward(self, x, context=None, mask=None, attn_map=None, prev_key=None):
        h = self.heads
        b = x.shape[0]
...
class BasicTransformerBlockST(nn.Module):
    ...
    def __init__(self, 
        ...
        **kwargs,
        ...
        if XFORMERS_IS_AVAILBLE:
            ...
            # self.attn1 = SparseCausalAttention(query_dim=dim, heads=n_heads, dim_head=d_head, dropout=dropout, **kwargs,)
    ...
    def forward(self, x, context=None, attn_map=None, **kwargs):
        return checkpoint(self._forward, (x, context, None, attn_map), self.parameters(), self.checkpoint)
        
    def _forward(self, x, context=None, mask=None, attn_map=None):
        ...
        if attn_map is not None:
            attn_self_map = attn_map['self']
            prev_key = attn_map['store_key']
        store_x = x[[-1],:,:].clone()
        self.store_x = store_x

        x = self.attn1(self.norm1(x), attn_map=attn_self_map, prev_key=prev_key) + x
        ...
        attn_cross_map = None
        if attn_map is not None:
            attn_cross_map = attn_map['cross']
        x = self.attn2(self.norm2(x), context=context_, attn_map=attn_cross_map) + x
        x = rearrange(x, '(b t) (h w) c -> b c t h w', b=b,h=h)
...
class SpatialTemporalTransformer(nn.Module):
    ...
    def forward(self, x, context=None, attn_map=None, **kwargs):
        ...
        for block in self.transformer_blocks:
            x = block(x, context=context, attn_map=attn_map, **kwargs)
...
class SparseCausalAttention(CrossAttention):
    def forward(self, x, context=None, mask=None, video_length=16, sequence_length=None, attn_map=None):
        to_out = self.to_out
        ...
        k = rearrange(k, "b n (h d) -> h b n d", b=video_length, h=h)
        k = torch.cat([k[:, [0] * video_length], k[:, former_frame_index]], dim=2)
        k = rearrange(k, "h b n d -> (b h) n d")

        v = rearrange(v, "b n (h d) -> h b n d", b=video_length, h=h)
        v = torch.cat([v[:, [0] * video_length], v[:, former_frame_index]], dim=2)
        v = rearrange(v, "h b n d -> (b h) n d")
        ...
        if mask is not None:
            if mask.shape[-1] != q.shape[1]:
                ...
                mask = F.pad(mask, (0, target_length), v=0.0)
                mask = mask.repeat_interleave(self.heads, dim=0)

        attn = sim.softmax(dim=-1)
        out = einsum('b i j, b j d -> b i d', attn, v)
        hidden_states = rearrange(out, '(b h) n d -> b n (h d)', h=self.heads)
        hidden_states = to_out(hidden_states)

        # # attention, what we cannot get enough of
        # if self._use_memory_efficient_attention_xformers:
        #     hidden_states = self._memory_efficient_attention_xformers(q, k, v, mask)
        #     # Some versions of xformers return output in fp32, cast it back to the dtype of the input
        #     hidden_states = hidden_states.to(q.dtype)
        # else:
        #     if self._slice_size is None or q.shape[0] // self._slice_size == 1:
        #         hidden_states = self._attention(q, k, v, mask)
        #     else:
        #         hidden_states = self._sliced_attention(q, k, v, sequence_length, dim, mask)
